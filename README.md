...
**작성 스타일** (Illustrated Transformer·Lil'Log 등 검증된 인기 패턴 — 2026-07 조사):
1. **산문이 기본, 불릿은 진짜 목록만** — 문단 2~4문장. 노트의 불릿 벽을 그대로 옮기지 않고
   문장으로 풀어 쓴다. 열거(데이터셋 목록, 한계 항목 등)만 불릿 유지.
2. **Running example** — 논문 리뷰는 구체 시나리오 하나(예: 공격 스토리)를 정해 도입에서 제시하고,
   방법·그림·결과 설명이 같은 시나리오를 관통하게 한다.
3. **주요 h2마다 시각 요소 1개 목표** — 다이어그램/표/스탯 중 하나. 설명 직후 배치(그림이 개념 전달의 본체).
4. **수식은 직관 먼저** — "말로 1~2문장(무엇을 왜) → 수식 → 기호 한 줄 해설" 순서. 수식 연속 나열 금지.
5. **질문형 전환** — 섹션 전환부에 독자가 던질 질문을 대신 던진다("그래서 몇 개로 줄었을까?").
6. **첫째·둘째 열거는 문단 분리** — "첫째, … 둘째, …"를 한 문단에 뭉치지 말고 항목마다 `<p>`를 나눈다.
7. **스탯 카드는 자기완결** — 카드(값+라벨+맥락)만 읽어도 뜻이 통해야 한다. 라벨은 지표명이 아니라
   "분석가가 열어봐야 할 경보 노드 수"처럼 독자 언어로, 맥락에 비교 대상·조건을 문장으로 쓴다.
